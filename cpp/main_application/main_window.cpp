...
void MainWindow::OnTimer(wxTimerEvent&)
{
    ...
    if(received_data)
    {
        ...
        internal::FunctionHeader hdr = received_data->getFunctionHeader();
        if(!hdr.hasType(internal::FunctionHeaderObjectType::FUNCTION))
        {
            std::cout << "Function missing!" << std::endl;
        }
