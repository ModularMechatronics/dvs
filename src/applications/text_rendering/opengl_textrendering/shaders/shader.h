...
private:
	// Member variables
	uint32_t s_id; // OpenGL program ID
	std::unordered_map<std::string,int> uniform_location_cache;
	std::string loadShaderSource(const char* fileName); // Function to load shader source from file
	uint32_t loadShader(GLenum type, const char* fileName); // Function to load a shader of given type
	void linkProgram(uint32_t vertexShader, uint32_t fragmentShader); // Function to link the shader program
...
public:
	...
	void setUniform(int i, uint32_t tid); // Function to set a texture uniform
