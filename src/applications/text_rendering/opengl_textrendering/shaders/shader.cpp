...
void shader::create_shader(const char* vertexFile, const char* fragmentFile)
{
	...
	uint32_t vertexShader = loadShader(GL_VERTEX_SHADER, vertexFile);

	// Load and compile the fragment shader
	uint32_t fragmentShader = loadShader(GL_FRAGMENT_SHADER, fragmentFile);
...
uint32_t shader::loadShader(GLenum type,const char* fileName)
{
	char infoLog[512];
	int success;

	uint32_t shader = glCreateShader(type); // Create a shader object of given type
...
void shader::setUniform(std::string name, float X, float Y) // Function to set a 2D float uniform
{
	...
	uint32_t uniformLocation = glGetUniformLocation(this->s_id, name.c_str()); // Get the uniform location using the provided name
...
void shader::setUniform(int i, uint32_t tid) // Function to set a texture uniform
{
	Bind(); // Use the shader program
