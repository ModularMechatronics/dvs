...
template <typename T> struct Plane
{
    ...
    Plane(const Point3D<T>& point, const Vec3<T>& normal_vector);
    ...
    std::pair<Point3D<T>, Vec3<T>> projectPointAndVectorOntoPlane(const Point3D<T>& point, const Vec3<T>& vec) const;
