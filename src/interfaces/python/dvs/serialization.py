...
def serialize_axes(axis_vecs):
    return np.array([np.float64(a) for a in (axis_vecs[0].x, axis_vecs[0].y, axis_vecs[0].z, axis_vecs[1].x, axis_vecs[1].y, axis_vecs[1].z)]).tobytes()
...
FUNCTION_HEADER_OBJECT_SERIALIZATION_FUNCTION = {
    ...
    CommunicationHeaderObjectType.DIMENSION_2D: lambda x: np.array([np.uint32(a) for a in x]).tobytes(),
    ...
    CommunicationHeaderObjectType.ELEMENT_NAME: lambda x: PropertyType.NAME.value.to_bytes(1, sys.byteorder) +\
...
SIZE_OF_FUNCTION_HEADER_OBJECT = {CommunicationHeaderObjectType.FUNCTION: None,
                                  CommunicationHeaderObjectType.NUM_BUFFERS_REQUIRED: None, 
                                  CommunicationHeaderObjectType.NUM_BYTES: None, 
                                  CommunicationHeaderObjectType.DATA_STRUCTURE: None, 
                                  CommunicationHeaderObjectType.BYTES_PER_ELEMENT: None, 
