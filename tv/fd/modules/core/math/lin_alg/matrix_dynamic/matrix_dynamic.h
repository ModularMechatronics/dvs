#ifndef MODULES_CORE_MATH_MATRIX_DYNAMIC_H_
#define MODULES_CORE_MATH_MATRIX_DYNAMIC_H_
...
template <typename T> Matrix<T>::Matrix(const std::vector<std::vector<T>>& vm)
{
    ...
    {
        ASSERT(vm[0].size() == vm[r].size()) << "All row vectors in input std vectors do not have the same size!";
    }
