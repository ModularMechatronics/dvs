	...
	*bemf = 0.;
	...
	case 11:
		*bemf = (max_bemf / (M_PI * (1./6.))) * (phase_thetae - (2. * M_PI));
		break;
	default:
		MC_ERROR("Angle out of bounds can not calculate bemf %f", phase_thetae);
		return GSL_FAILURE;
	}
    

#ifdef DEBUG
	MC_DEBUG("alpha %.5e bemf %.5e\n", phase_thetae, *bemf);
#endif

	return GSL_SUCCESS;
}

void updateVoltages(double t, struct state_vector *sv, struct command_vector *cv, struct motor *m, struct voltages_vector *vv)
{
...
int modelDynamics(double t, const double asv[], double aov[], void *params)
{
...
#ifdef DEBUG
	MC_DEBUG("Input t %.5e iu %.5e iv %.5e ew %5e th %5e om %5e\n", t, sv->iu, sv->iv, sv->iw, sv->theta, sv->omega);
#endif
...
#ifdef DEBUG
	MC_DEBUG("damping %.5e omega %.5e torque %.5e mtorque %.5e\n", p->m->damping, sv->omega, p->pv->torque, mtorque);
#endif
	...
	/* Calculate voltages. */
	updateVoltages(t, sv, p->cv, p->m, &vv);
